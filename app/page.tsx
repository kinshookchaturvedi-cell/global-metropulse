...
import { useMetroStats } from '@/features/projects/hooks/useMetroStats';

export default function Home() {
  const { selectedCity } = useUiStore();
  const { widgets } = useDashboardLayoutStore();
  const { data: projects = [], isLoading } = useMetroStats(selectedCity);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-12">
        ...
        {/* Projects Section - Using React Query Data */}
        <div className="mb-16">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-slate-800/50 rounded-2xl h-48 animate-pulse"></div>
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.map((project, i) => (
                <div key={project.id || i} className="group relative bg-slate-800/50 backdrop-blur-sm rounded-2xl overflow-hidden border border-slate-700/50 hover:border-slate-600 transition-all duration-300 hover:transform hover:scale-105">
                  <div className={`h-2 w-full bg-gradient-to-r ${
                    project.color === 'blue' ? 'from-blue-500 to-cyan-400' : 
                    project.color === 'yellow' ? 'from-yellow-500 to-orange-400' : 
      ...
      <div className="text-center py-8 border-t border-slate-700/50">
        <p className="text-gray-500 text-sm">
          Powered by React Query & Zustand • Real-time metro intelligence platform
        </p>
        <p className="text-gray-600 text-xs mt-2">
          Data updates automatically • Built with Next.js, React Query & Tailwind CSS
        </p>
      </div>
    </div>
  );
